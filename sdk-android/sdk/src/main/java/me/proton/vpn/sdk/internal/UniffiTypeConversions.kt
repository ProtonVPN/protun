...
import me.proton.vpn.sdk.api.ConnectionStats
import me.proton.vpn.sdk.api.PacketCaptureStopReason
...
fun PeerConnectionInfo.toSdk(): PeerConnection =
    PeerConnection(
        protocol = protocol.toSdk(),
        id = peerId,
        entryAddr = InetSocketAddress(entryIp, port.toInt())
    )

fun DisconnectReason.toSdk(): VpnDisconnectError = when (this) {
    is DisconnectReason.TunEstablishError -> VpnDisconnectError.TunInterfaceError(message)
}

fun Protocol.toSdk(): VpnProtocol = when (this) {
...
fun CaptureStopReason.toSdk(): PacketCaptureStopReason = when (this) {
    CaptureStopReason.AlreadyStopped -> PacketCaptureStopReason.AlreadyStopped
    is CaptureStopReason.Disconnected -> PacketCaptureStopReason.Disconnected(file.toSdk().file)
    is CaptureStopReason.MaxSizeReached -> PacketCaptureStopReason.MaxSizeReached(file.toSdk().file)
    is CaptureStopReason.Request -> PacketCaptureStopReason.Request(file.toSdk().file)
}

fun PcapFileInfo.toSdk() = PacketCaptureInfo(
    when (val f = file) {
...
fun Event.ConnectionStats.toSdk() = ConnectionStats(
